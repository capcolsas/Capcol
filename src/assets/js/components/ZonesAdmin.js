...
export const ZonesAdmin=(mount,deps={})=>{
  ...
  async function openCreateModal(){
    const modal=await showActionModal({
      ...
      fields:[{ id:'name', label:'Nombre', type:'text', required:true, placeholder:'Nombre de la zona' }]
    });
  ...
  function actionsCell(z){ const box=el('div',{className:'row-actions'},[]); const btnEdit=el('button',{className:'btn btn--icon',title:'Editar'},['\u270E']); btnEdit.addEventListener('click',()=> openEditModal(z)); const btnToggle=el('button',{className:'btn btn--icon '+(z.estado==='activo'?'btn--danger':'' ),title:z.estado==='activo'?'Desactivar':'Activar','aria-label':z.estado==='activo'?'Desactivar':'Activar'},[ z.estado==='activo'?'\u23FB':'\u21BA' ]); btnToggle.addEventListener('click',async()=>{ const target=z.estado==='activo'?'inactivo':'activo'; const modal=await showActionModal({ title:`${target==='inactivo'?'Desactivar':'Activar'} zona`, message:`Zona: ${z.nombre||'-'}`, confirmText:target==='inactivo'?'Desactivar':'Activar', fields:[{ id:'detail', label:'Detalle', type:'textarea', required:true, placeholder:'Escribe el motivo o detalle de esta accion' }] }); if(!modal.confirmed) return; try{ await deps.setZoneStatus?.(z.id,target); await deps.addAuditLog?.({ targetType:'zone', targetId:z.id, action: target==='activo'?'activate_zone':'deactivate_zone', before:{estado:z.estado}, after:{estado:target}, note: modal.values.detail||null }); }catch(e){ alert('Error: '+(e?.message||e)); } }); const btnInfo=el('button',{className:'btn btn--icon',title:'Ver informacion','aria-label':'Ver informacion'},['\u24D8']); btnInfo.addEventListener('click',()=>{ const info=auditInfoData(z); showInfoModal('Informacion del registro',[`Evento: ${info.action}`,`Usuario: ${info.user}`,`Fecha: ${info.date}`]); }); box.append(btnEdit,btnToggle,btnInfo); return box; }
