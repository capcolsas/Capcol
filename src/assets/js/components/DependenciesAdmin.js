...
export const DependenciesAdmin=(mount,deps={})=>{
  ...
  function render(){ const term=search(); const st=filterStatus(); const data=snapshot.filter(d=> ((!term||(d.codigo||'').toLowerCase().includes(term)||(d.nombre||'').toLowerCase().includes(term)) && (!st || d.estado===st))); const sorted=sortData(data); const pageRows=paginator.slice(sorted); tbody.replaceChildren(...pageRows.map(d=> row(d))); cards.replaceChildren(...(pageRows.length?pageRows.map(d=> recordCard(d,{title:d.nombre||'-',subtitle:`Codigo: ${d.codigo||'-'}`,meta:[['Estado',d.estado||'-']],actions:actionsCell(d)})):[el('p',{className:'text-muted record-card__empty'},['Sin dependencias para mostrar.'])])); const msg=qs('#msg',ui); if(msg) msg.textContent=`Total registros filtrados: ${data.length}`; updateSortIndicators(); }
  ...
  function actionsCell(d){ const box=el('div',{className:'row-actions'},[]); const btnEdit=el('button',{className:'btn btn--icon',title:'Editar'},['\u270E']); btnEdit.addEventListener('click',()=> openEditModal(d)); const btnToggle=el('button',{className:'btn btn--icon '+(d.estado==='activo'?'btn--danger':'' ),title:d.estado==='activo'?'Desactivar':'Activar','aria-label':d.estado==='activo'?'Desactivar':'Activar'},[ d.estado==='activo'?'\u23FB':'\u21BA' ]); btnToggle.addEventListener('click',async()=>{ const target=d.estado==='activo'?'inactivo':'activo'; const modal=await showActionModal({ title:`${target==='inactivo'?'Desactivar':'Activar'} dependencia`, message:`Dependencia: ${d.nombre||'-'}`, confirmText:target==='inactivo'?'Desactivar':'Activar', fields:[{ id:'detail', label:'Detalle', type:'textarea', required:true, placeholder:'Escribe el motivo o detalle de esta accion' }] }); if(!modal.confirmed) return; try{ await deps.setDependencyStatus?.(d.id,target); await deps.addAuditLog?.({ targetType:'dependency', targetId:d.id, action: target==='activo'?'activate_dependency':'deactivate_dependency', before:{estado:d.estado}, after:{estado:target}, note: modal.values.detail||null }); }catch(e){ alert('Error: '+(e?.message||e)); } }); const btnInfo=el('button',{className:'btn btn--icon',title:'Ver informacion','aria-label':'Ver informacion'},['\u24D8']); btnInfo.addEventListener('click',()=>{ const info=auditInfoData(d); showInfoModal('Informacion del registro',[`Evento: ${info.action}`,`Usuario: ${info.user}`,`Fecha: ${info.date}`]); }); box.append(btnEdit,btnToggle,btnInfo); return box; }
